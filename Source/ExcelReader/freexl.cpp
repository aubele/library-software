...
static void swap16 (biff_word16 * word)
{
...
static void swap32 (biff_word32 * word)
{
...
static void swap_float (biff_float * word)
{
...
static int biff_set_utf8_converter (biff_workbook * workbook)
{
...
static char * convert_to_utf8 (iconv_t converter, const char *buf, int buflen, int *err)
{
...
static void get_unicode_params (unsigned char *addr, int swap, unsigned int *start_offset,
...
static int parse_unicode_string (iconv_t converter, unsigned short characters,
...
static int decode_rk_integer (unsigned char *bytes, int *value, int swap)
{
...
static int decode_rk_float (unsigned char *bytes, double *value, int swap)
{
...
static int check_xf_datetime (biff_workbook * workbook, unsigned short xf_index,
...
static int check_xf_datetime_58 (biff_workbook * workbook, unsigned short xf_index,
		      int *is_date, int *is_datetime, int *is_time)
{
/* 
/ testing for DATE/DATETIME/TIME formats 
...
static void compute_time (int *hh, int *mm, int *ss, double percent)
{
...
/* allocating the FAT chain */
    fat_chain *chain = (fat_chain*)malloc (sizeof (fat_chain));
    if (!chain)
	return NULL;
    chain->swap = swap;
    if (sector_shift == 12)
...
/* allocating the cell values array */
    ...
    workbook->active_sheet->cell_values =
    (biff_cell_value*) malloc (sizeof (biff_cell_value) *
...
/* appending a further Sheet to the Workbook */
    biff_sheet *sheet = (biff_sheet *) malloc (sizeof (biff_sheet));
    if (!sheet)
	return 0;
...
/* allocating and initializing the Workbook struct */
    biff_workbook *workbook = (biff_workbook *) malloc (sizeof (biff_workbook));
    if (!workbook)
	return NULL;
...
/* inserting a sector into the FAT chain [linked list] */
    fat_entry *entry = (fat_entry *) malloc (sizeof (fat_entry));
    if (entry == NULL)
	return FREEXL_INSUFFICIENT_MEMORY;
...
/* allocating the FAT sectors array */
    chain->fat_array = (fat_entry **) malloc (sizeof (fat_entry *) * chain->fat_array_count);
...
/* loading in memory the whole ministream */
    ...
    miniStream = (unsigned char*) malloc (workbook->fat->miniFAT_len);
    if (miniStream == NULL)
      {
...
/* attempting to parse a Shared String Table */
	...
	&& workbook->shared_strings.utf8_strings == NULL)
      ...
      workbook->shared_strings.utf8_strings = (char**)
    ...
    for (i_string = workbook->shared_strings.current_index;
	 i_string < workbook->shared_strings.string_count; i_string++)
      ...
      workbook->shared_strings.current_utf16_buf = (char*)
